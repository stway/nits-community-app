...
export default function Creative(){
  ...
  useEffect(()=>{
    ...
    async function load(){
      try {
        const r = await fetch(`${API}/api/posts?type=creative`);
  ...
  return (
    <>
      ...
      <div className="container">
        <div className="section-hero" style={{backgroundImage:"url(/images/creative.jpg)",backgroundSize:"cover"}}>Creativity</div>
        <Composer apiBase={API} section="creative" onPosted={async ()=>{ const r = await fetch(`${API}/api/posts?type=creative`); const j = await r.json(); setPosts(j.posts||[]); }} />

...
export default function PhotoDump(){
  ...
  useEffect(() => {
    ...
    async function load(){
      try {
        const r = await fetch(`${API}/api/posts?type=photodump`);
  ...
  return (
    <>
      ...
      <div className="container">
        <div className="section-hero" style={{backgroundImage:"url(/images/photodump.jpg)",backgroundSize:"cover"}}>Photo Dump</div>
        <Composer apiBase={API} section="photodump" onPosted={async ()=>{ const r = await fetch(`${API}/api/posts?type=photodump`); const j = await r.json(); setPosts(j.posts||[]); }} />

...
export default function Confessions(){
  ...
  useEffect(()=>{
    ...
    async function load(){
      try {
        const r = await fetch(`${API}/api/posts?type=confession`);
  ...
  return (
    <>
      ...
      <div className="container">
        <div className="section-hero" style={{backgroundImage:"url(/images/confession.jpg)",backgroundSize:"cover"}}>Confessions</div>
        <Composer apiBase={API} section="confession" onPosted={async ()=>{ const r = await fetch(`${API}/api/posts?type=confession`); const j = await r.json(); setPosts(j.posts||[]); }} />

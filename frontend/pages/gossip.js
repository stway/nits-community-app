...
export default function Gossip(){
  ...
  useEffect(()=>{
    ...
    async function load(){
      try {
        const r = await fetch(`${API}/api/posts?type=gossip`);
  ...
  return (
    <>
      ...
      <div className="container">
        <div className="section-hero" style={{backgroundImage:"url(/images/gossip.jpg)",backgroundSize:"cover"}}>Gossip</div>
        <Composer apiBase={API} section="gossip" onPosted={async ()=>{ const r = await fetch(`${API}/api/posts?type=gossip`); const j = await r.json(); setPosts(j.posts||[]); }} />
